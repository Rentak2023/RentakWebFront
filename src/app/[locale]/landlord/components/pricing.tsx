import { clsx } from "clsx";
import { CheckIcon } from "lucide-react";
import { useTranslations } from "next-intl";

type Tier = {
  name: string;
  id: string;
  href: string;
  price: { monthly: string; annually: string } | string;
  description?: string;
  features: Array<string>;
  featured: boolean;
  cta: string;
};

export default function Pricing() {
  const t = useTranslations("landlord.plans");

  const tiers: Array<Tier> = [
    {
      name: t("rentak-collect.name"),
      id: "tier-collect",
      href: "/rent-collection",
      price: "3%",
      features: [
        t("rentak-collect.features.serviceCharge"),
        t("rentak-basic.features.paymentReminders"),
        t("rentak-basic.features.multiplePaymentMethods"),
        t("rentak-basic.features.directTransfers"),
      ],
      featured: false,
      cta: t("rentak-collect.cta"),
    },
    {
      name: t("rentak-basic.name"),
      id: "tier-basic",
      href: "/rentak-basic",
      price: "5%",
      features: [
        t("rentak-basic.features.paymentReminders"),
        t("rentak-basic.features.multiplePaymentMethods"),
        t("rentak-basic.features.marketingAndPromotion"),
        t("rentak-basic.features.legalContracts"),
        t("rentak-basic.features.directTransfers"),
        t("rentak-basic.features.rentReports"),
        t("rentak-basic.features.propertyHandoverReports"),
      ],
      featured: false,
      cta: t("rentak-basic.cta"),
    },
    {
      name: t("rentak-secure.name"),
      id: "tier-secure",
      href: "/rentak-secure",
      price: "10%",
      features: [
        t("rentak-secure.features.paymentReminders"),
        t("rentak-secure.features.multiplePaymentMethods"),
        t("rentak-secure.features.marketingAndPromotion"),
        t("rentak-secure.features.legalContracts"),
        t("rentak-secure.features.directTransfers"),
        t("rentak-secure.features.guaranteedRent"),
        t("rentak-secure.features.propertyProtection"),
        t("rentak-secure.features.completeUnitCare"),
      ],
      featured: true,
      cta: t("rentak-secure.cta"),
    },
  ];

  return (
    <div className="mt-44 bg-white">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <h2 className="sr-only">{t("plans")}</h2>
          <p className="text-primary-900 mt-2 text-pretty text-4xl font-semibold tracking-tight sm:text-5xl">
            {t("title")}
          </p>
        </div>
        <p className="mx-auto mt-6 max-w-2xl text-center text-lg leading-8 text-slate-600">
          {t("subtitle")}
        </p>
        <div className="isolate mx-auto mt-10 grid max-w-md grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {tiers.map((tier) => (
            <div
              key={tier.id}
              className={clsx(
                tier.featured
                  ? "bg-primary-800 ring-primary-800"
                  : "ring-slate-200",
                "rounded-3xl p-8 ring-1 xl:p-10",
              )}
            >
              <div className="flex items-center justify-between gap-x-4">
                <h3
                  id={tier.id}
                  className={clsx(
                    tier.featured ? "text-white" : "text-primary-800",
                    "text-xl font-medium leading-8",
                  )}
                >
                  {tier.name}
                </h3>
                {tier.featured && (
                  <p className="text-primary-800 rounded-full bg-white/50 px-2.5 py-1 text-xs font-semibold leading-5">
                    {t("most-popular")}
                  </p>
                )}
              </div>
              {/* <p
                className={clsx(
                  tier.featured ? "text-slate-300" : "text-slate-600",
                  "mt-4 text-sm leading-6",
                )}
              >
                {tier.description}
              </p> */}
              {/* <p className="mt-6 flex items-baseline gap-x-1">
                <span
                  className={clsx(
                    tier.featured ? "text-white" : "text-primary-800",
                    "text-6xl font-semibold tracking-tight",
                  )}
                >
                  {typeof tier.price === "string"
                    ? tier.price
                    : tier.price[frequency.value]}
                </span>
                {typeof tier.price === "string" ? null : (
                  <span
                    className={clsx(
                      tier.featured ? "text-slate-300" : "text-slate-600",
                      "text-sm font-semibold leading-6",
                    )}
                  >
                    {frequency.priceSuffix}
                  </span>
                )}
              </p> */}
              <a
                href={tier.href}
                aria-describedby={tier.id}
                className={clsx(
                  tier.featured
                    ? "text-primary-800 bg-white/80 hover:bg-white/60 focus-visible:outline-white"
                    : "bg-primary-800 shadow-xs hover:bg-primary-800/90 focus-visible:outline-primary-600 text-white",
                  "mt-6 block rounded-md px-3 py-2 text-center text-sm font-semibold leading-6 focus-visible:outline-2 focus-visible:outline-offset-2",
                )}
              >
                {tier.cta}
              </a>
              <ul
                role="list"
                className={clsx(
                  tier.featured ? "text-slate-300" : "text-slate-600",
                  "mt-8 space-y-3 text-sm leading-6 xl:mt-10",
                )}
              >
                {tier.features.map((feature) => (
                  <li key={feature} className="flex gap-x-3">
                    <CheckIcon
                      aria-hidden="true"
                      className={clsx(
                        tier.featured ? "text-white" : "text-primary-600",
                        "h-6 w-5 flex-none",
                      )}
                    />
                    {feature}
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}
