...
function StepperProvider({ value, children }: StepperContextProviderProps) {
  ...
  return <StepperContext value={contextValue}>{children}</StepperContext>;
}
...
const Stepper = ({
  ...
  ...rest
}: StepperProps & { ref?: React.RefObject<HTMLDivElement> }) => {
    ...
    >
      <div
        ...
        style={
          {
            "--step-icon-size":
              variables?.["--step-icon-size"] ?? VARIABLE_SIZES[size],
    ...
    <>
      {React.Children.map(childArr[activeStep], (node) => {
        ...
        return React.Children.map(
          (node as React.ReactElement<FullStepProps>).props.children,
          (childNode) => childNode,
        );
      })}
    </>
...
const Step = ({
  ref,
  ...props
}: StepProps & { ref?: React.RefObject<any> }) => {
  const {
    children,
    description,
    icon,
...
const VerticalStep = ({
  ref,
  ...props
}: VerticalStepProps & { ref?: React.RefObject<HTMLDivElement> }) => {
  const {
    children,
    index,
    isCompletedStep,
...
const HorizontalStep = ({
  ref,
  ...props
}: StepSharedProps & { ref?: React.RefObject<HTMLDivElement> }) => {
  const {
    isError,
    isLoading,
    onClickStep,
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        "data-[active=true]:border-green-600 data-[active=true]:bg-green-600 data-[active=true]:text-slate-100",
        "data-[current=true]:border-primary-800 data-[current=true]:bg-primary-800 data-[current=true]:text-slate-100",
        "data-[invalid=true]:border-red-600 data-[invalid=true]:bg-red-600 data-[invalid=true]:text-slate-100",
...
const StepIcon = ({
  ref,
  ...props
}: StepIconProps & { ref?: React.RefObject<HTMLDivElement> }) => {
